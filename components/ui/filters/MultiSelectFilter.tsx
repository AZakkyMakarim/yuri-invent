        ...
        <div ref={dropdownRef} className={cn('relative h-9', className)}>
            ...
            <button
                ...
                className={cn(
                    ...
                    hasSelection && 'border-[var(--color-primary)]/50 bg-[var(--color-primary)]/5'
                )}
            >
                <span className={cn(
                    'flex-1 text-left truncate',
                    hasSelection ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-muted)]'
                )}>
                    {displayValue}
                </span>
                {hasSelection ? (
            ...
            {/* Dropdown */}
            {isOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 z-50 min-w-[180px] bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg shadow-lg py-1 animate-fadeIn">
                    {/* Search Input */}
                    {searchable && (
                        ...
                        {filteredOptions.length === 0 ? (
                            ...
                            filteredOptions.map((option) => (
                                <button
                                    ...
                                    className="w-full px-3 py-2 text-sm text-left hover:bg-[var(--color-bg-hover)] flex items-center gap-2"
                                >
                                    <div
                                        className={cn(
